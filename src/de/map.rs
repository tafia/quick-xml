...
where
    ...
    E: EntityResolver,
{
    fn unsupported<T>(&self, typ: &'static str) -> Result<T, DeError> {
        Err(DeError::Custom(format!(
            "cannot deserialize {typ} as a mapping of XML attributes. Only structs are supported for $attributes fields"
        )))
    }
}
