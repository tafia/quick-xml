...
use crate::escape::{
    escape, normalize_attribute_value, resolve_predefined_entity, resolve_xml_entity,
};
...
impl<'a> Attribute<'a> {
    ...
    pub fn normalized_value(&self) -> XmlResult<Cow<'a, str>> {
        // resolve_xml_entity returns only non-recursive replacements, so depth=1 is enougth
        self.normalized_value_with(1, resolve_xml_entity)
    }
    ...
    /// [`resolve_xml_entity`] in your function.
    ///
    /// This will allocate unless the raw attribute value does not require normalization.
    ///
    /// Note, althougth you may use this library to parse HTML, you cannot use this
    ...
    /// [the XML specification]: https://www.w3.org/TR/xml11/#AVNormalize
    /// [only for]: https://html.spec.whatwg.org/#normalize-newlines
    pub fn decoded_and_normalized_value(&self, decoder: Decoder) -> XmlResult<Cow<'a, str>> {
        // resolve_xml_entity returns only non-recursive replacements, so depth=1 is enougth
        self.decoded_and_normalized_value_with(decoder, 1, resolve_xml_entity)
    }
    ...
    pub fn unescape_value(&self) -> XmlResult<Cow<'a, str>> {
        // resolve_predefined_entity returns only non-recursive replacements, so depth=1 is enougth
        self.normalized_value_with(1, resolve_predefined_entity)
    }
    ...
    pub fn unescape_value_with<'entity>(
        ...
        self.normalized_value_with(128, resolve_entity)
    }
    ...
    pub fn decode_and_unescape_value(&self, decoder: Decoder) -> XmlResult<Cow<'a, str>> {
        // resolve_predefined_entity returns only non-recursive replacements, so depth=1 is enougth
        self.decoded_and_normalized_value_with(decoder, 1, resolve_predefined_entity)
    }
    ...
    pub fn decode_and_unescape_value_with<'entity>(
        ...
        self.decoded_and_normalized_value_with(decoder, 128, resolve_entity)
    }
...
mod xml {
    ...
    mod attribute_value_normalization {
        ...
        fn empty() {
            ...
            assert_eq!(
                attr.decoded_and_normalized_value(Decoder::utf8()).unwrap(),
                Cow::Borrowed("")
            );
        }
        ...
        fn space_replacement() {
            ...
            assert_eq!(
                attr.decoded_and_normalized_value(Decoder::utf8()).unwrap(),
                Cow::<str>::Owned(" foo   bar baz  delta  ".to_string())
            );
        }
        ...
        fn unterminated_entity() {
            ...
            let attr = Attribute::from(("foo".as_bytes(), raw_value));
            match attr.decoded_and_normalized_value(Decoder::utf8()) {
        ...
        fn entity_replacement() {
            ...
            assert_eq!(
                attr.decoded_and_normalized_value_with(Decoder::utf8(), 5, &custom_resolver)
                    .unwrap(),
                Cow::<str>::Owned("\r\rA\n \nB\r\n".to_string())
            );
        }
