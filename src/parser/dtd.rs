...
pub enum DtdParser {
    ...
    /// Otherwise, it holds `None` (this is an initial state).
    ///
    /// ```text
...
impl DtdParser {
                        ...
                        match b {
                            ...
                            b'>' => {
                                *self = Self::Finished;
                                ...
                                // +1 for `>`
                                return Ok(Some((&chunk[..len], len + 1)));
                            }
                            _ => {}
                        }
                        continue;
    ...
    #[inline]
    fn switch(&mut self, markup: &[u8]) -> Result<Option<usize>, SyntaxError> {
        match markup {
            [b'?', ..] => {
                // <?
            ...
            // next iteration.
            _ if markup.len() < 9 => Ok(None),
            _ => Err(SyntaxError::UnknownDtdMarkup),
        }
    }
}
