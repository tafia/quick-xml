//! Contains serializer for a special `&data` field
...
/// A serializer used to serialize a `$cdata` field of a struct or map.
///
...
pub struct CDataSerializer<W: Write>(pub SimpleTypeSerializer<W>);

impl<W: Write> Serializer for CDataSerializer<W> {
    type Ok = W;
    type Error = SeError;
